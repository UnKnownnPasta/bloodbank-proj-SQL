...
def display_error():
    ...
    error_count += 1
    canvas.itemconfigure(error_text, fill='white', text=f'Invalid Login Details. [{error_count}]')
...
def welcome_screen(src):
    ...
    temporary_button = Button(src, command=lambda: switch_to_entry(src, images),
                              bd=0, activebackground='#D22B2B', bg='#EE4B2B', relief="flat", image=images[2])
