...
def donation_choice(src, ID):
    global images, id
    id = ID
    ttl = Label(src, text='  Choose a option:', font=('Cascadia Code', 22), width=55, bg='#FBFCF8', height=1, anchor='w')
    ttl.place(x=0, y=20)
    ...
    opt_1 = Button(src, text='Donate', command=lambda: (destroy_frameitems(src), donate_blood_citizen(src=src)),
            bd=0, activebackground='#D22B2B', bg='#EE4B2B', relief="flat", padx=141, pady=8, fg='white', font=('Cascadia Code', 15))
    opt_2 = Button(src, text='Transfer', command=lambda: (destroy_frameitems(src), donate_blood_hospital(src=src)),
...
def donate_blood_citizen(src):
    ttl = Label(src, text='  Donating Blood', font=('Cascadia Code', 22), width=55, bg='#FBFCF8', height=1, anchor='w')
    ttl.place(x=0, y=20)


def donate_blood_hospital(src):
    ttl = Label(src, text='  Transferring Blood', font=('Cascadia Code', 22), width=55, bg='#FBFCF8', height=1, anchor='w')
    ttl.place(x=0, y=20)
